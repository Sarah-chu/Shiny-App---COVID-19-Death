...
EU27DT = coronaD[coronaD$Country %in% EU27]
...
#------------------------------------------------------------
# Create a ggplot for illustrating the death of each country
#------------------------------------------------------------

# Select three countries to test the ggplot
SelectedCountries = c("France","Italy","Spain")

# Create a ggplot to show the death of a particular country
ggplot(PlotDT[Country %in% SelectedCountries] , aes(x= Date, y = Dead, group = Country)) + 
    geom_line(colour ="grey") + # All of the unselected countries are in grey color
    geom_line(data = PlotDT[Country == "Spain"] , colour = "Blue") + # The geometry line of Spain is blue
    ggtitle("Deaths from Covid 19", subtitle = "Data: John Hopkins") + # Add a title and a subtitle
    scale_x_date(limits = as.Date(c("2020-02-15","2020-04-07"))) + # Restrict the scale on the x-axis
    scale_y_log10(name = "Deaths (logarithmic scale)") + # Change the scale on the y-axis to logarithmic
    theme(text = element_text(family = "Times New Roman")) # Change the layout of the plot
...
#------------------------------------------------------------
# Create a ggplot for illustrating the Death per Capita of each country
#------------------------------------------------------------
  
# Create a plot for Death per Capita 
    ggplot(PlotPCDT[`Country` %in% SelectedCountries],
           aes(x= Date, y = Dead, group = `Country`)) + 
    geom_line(colour ="grey") + # All other unselected countries are in grey color
    geom_line(data = PlotPCDT[`Country` == "Spain" ],
              colour = "Blue") +  # The geometry line of Spain is in blue color
    ggtitle("Deaths per capita from Covid 19", subtitle = "Data: John Hopkins") + # Add a title and a subtitle
    scale_x_date(limits = as.Date(c("2020-02-15","2020-04-07"))) + # Restrict the scale on the x-axis
    scale_y_log10(name = "Deaths (logarithmic scale)") + # Change the scale on the y-axis to logarithmic
    theme(text = element_text(family = "Times New Roman")) # Add some of the layout-elements
